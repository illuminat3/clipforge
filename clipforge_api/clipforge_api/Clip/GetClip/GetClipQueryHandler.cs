...
{
    public class GetClipQueryHandler(AppDbContext db, IHttpContextAccessor httpContextAccessor) : IRequestHandler<GetClipQuery, GetClipResult>
    {
        public async Task<GetClipResult> Handle(GetClipQuery request, CancellationToken ct)
        {
            ...
            var clip = await db.Clips.FirstOrDefaultAsync(c => c.Id == clipId && c.UserId == userId, ct) ?? throw new KeyNotFoundException("Clip not found.");
